...
**Status**: draft · **Created**: 2022-06-22 · **Last sync**: 2026-06-27 (transactions-ui)
...
> **Delta spec — REQ-TX-15 REPLACED.** This file mirrors the
> canonical `transactions` capability spec at
> `openspec/specs/transactions/spec.md` and applies a single
> delta: **REQ-TX-15 (the "three smoke pages" requirement) is
> REPLACED** by a thin reference to the new `ui` capability
> (`openspec/specs/ui/spec.md`), which owns the production
> UI requirements. All other requirements (REQ-TX-1 to
> REQ-TX-14) are preserved verbatim from the canonical. No
> BR changes; no behavior changes in the Hono routes; no
> data model changes. `sdd-archive` lifts this delta into the
> canonical at `openspec/specs/transactions/spec.md` and
> removes the smoke wording of REQ-TX-15.
>
> The spec declares **what MUST be true** after the change
> lands, not how to implement it. Implementation details
> (file paths, schema syntax, test layout) are limited to
> what the cross-module contract requires.

## Changed in `transactions-ui`

| Requirement  | Change    | Detail                                                                                                                                                                      |
| ------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-TX-15    | REPLACED  | Was "Three smoke pages mirror the accounts slice" (smoke UI). Replaced by a thin pointer to `openspec/specs/ui/spec.md` REQ-UI-1 to REQ-UI-11, which own the production UI. |
| REQ-TX-1..14 | UNCHANGED | All other requirements preserved verbatim from `openspec/specs/transactions/spec.md` (last sync 2026-06-22, transactions).                                                  |

The replacement decouples the user-facing surface from this
spec so future UI evolution (`ui-dark-mode`, `ui-i18n`,
`ui-charts`) lands as additions to the `ui` capability, not
as further revisions of REQ-TX-N here.
...
is a snapshot, never a mutation of native state; (d) the
production UI surface is owned by the `ui` capability
(`openspec/specs/ui/spec.md`), not by this spec.
...
(`reports`, `snapshots`, the `ui` capability) can read or
subscribe to via `TransactionRepositoryPort` and the
`TransactionRecorded` domain event without learning the
upstream details.
...
- **User-facing UI surface**: owned by the `ui` capability
  (`openspec/specs/ui/spec.md`). This spec no longer defines
  page-level requirements; see `Changed in transactions-ui`
  above for the delta on REQ-TX-15.
...
- **Budget rules / spending limits.** Out of v1 (`reports`
  territory).
- **Production UI.** Owned by the `ui` capability
  (`openspec/specs/ui/spec.md`). This spec references the
  `ui` REQs but does not duplicate them.
...
- `ui` (separate capability) owns the design-system
  primitives (`app/_ui/`) and the production page renders
  (`app/transactions/**`, `app/dashboard/**`,
  `app/accounts/**`). The two flags `include=lastActivity` and
  `include=accountName` are additive on the existing GET
  endpoints (see `ui/spec.md` REQ-UI-1 and REQ-UI-2); the
  endpoints without the flag are unchanged.
...
- Production UI (`ui` capability owns the production surface;
  this spec references it but does not duplicate it).
- Mobile app.
...
- **Proposal**: `openspec/changes/transactions/proposal.md` —
  ...
  forecast.
- **Proposal (delta)**: `openspec/changes/transactions-ui/proposal.md` —
  the upstream change that introduced the production UI and
  added BR-UI-1 to BR-UI-9. The `ui` capability spec lives at
  `openspec/changes/transactions-ui/specs/ui/spec.md` and is
  promoted to canonical by `sdd-archive`. REQ-TX-15 in this
  spec points at REQ-UI-1 to REQ-UI-11 in that spec; future
  UI evolution lands as additions to the `ui` capability.
- **UI capability spec**: `openspec/specs/ui/spec.md` —
  REQ-UI-1 to REQ-UI-11 own the production UI surface,
  including the two additive query flags and the WCAG 2.2 AA
  a11y floor.
...
- **Auth spec**: `openspec/specs/auth/spec.md` — the
  ...
  the caller" rule. The `transactions` capability follows this
...
- **2026-06-27 (transactions-ui)** — REQ-TX-15 REPLACED. The
  ...
  dependencies. The `ui` capability is promoted from
...
- `openspec/changes/transactions/explore.md` — upstream research
  (15 DG-TX-N + 4 open questions, ~50 file:line citations).
- `openspec/changes/transactions-ui/proposal.md` — proposal v1
  (2026-06-27) with BR-UI-1 to BR-UI-9 and the four locked
  open questions.
