...
**Status**: draft · **Created**: 2026-06-27 · **Last sync**: n/a
...
> First write of the `ui` capability spec. It operationalizes
> the `transactions-ui` proposal v1 (draft 2026-06-27). The spec
> declares **what MUST be true** after the change lands, not how
> to implement it. Implementation details (file paths, component
> names, schema syntax, test layout) are limited to what the
> cross-module contract requires.
>
> This is the **delta spec** for the new `ui` capability. The
> `ui` capability does not yet exist under `openspec/specs/`
> (the slot is reserved at `openspec/config.yaml:15`); it lives
> under `openspec/changes/transactions-ui/specs/ui/` until
> `sdd-archive` promotes it to the canonical location
> `openspec/specs/ui/spec.md`. The canonical and the delta are
> kept in lockstep; the canonical is the source of truth.
