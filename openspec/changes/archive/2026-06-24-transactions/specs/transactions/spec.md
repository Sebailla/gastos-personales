...
> contract requires.
>
> This is a **delta spec** for the new `transactions` capability.
> The `transactions` capability does not yet exist under
> `openspec/specs/` — it lives under
> `openspec/changes/transactions/specs/transactions/` until
...
codes defined in `Error codes` below. (Traces: BR-TX-1 to
BR-TX-11, DG-TX-9.)
