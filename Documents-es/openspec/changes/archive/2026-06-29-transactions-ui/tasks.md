...
- **Scope (in)**:
  - Tabla de tokens en `app/_ui/tokens.css` (tema light + scope
    ...
    (`--ui-rounded-sm (or -md, -lg, -full)`), elevación
    (`--ui-shadow-sm (or -md, -lg)`), escala de tipografía
    (`--ui-text-xs (or -sm, -base, -lg, -xl, -2xl, -3xl)` +
    `--ui-font-normal (or -medium, -semibold, -bold)`).
