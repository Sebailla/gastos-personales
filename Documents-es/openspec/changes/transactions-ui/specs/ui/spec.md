...
**Estado**: draft · **Creado**: 2026-06-27 · **Última sincronización**: n/a
...
> Primera escritura del spec de la capability `ui`. Operacionaliza
> la propuesta de `transactions-ui` v1 (draft 2026-06-27). El spec
> declara **lo que DEBE ser verdadero** después de que el cambio
> aterrice, no cómo implementarlo. Los detalles de implementación
> (paths de archivo, nombres de componentes, sintaxis de schemas,
> layout de tests) se limitan a lo que requiere el contrato
> cross-module.
>
> Este es el **spec delta** para la nueva capability `ui`. La
> capability `ui` todavía no existe bajo `openspec/specs/` (el slot
> está reservado en `openspec/config.yaml:15`); vive bajo
> `openspec/changes/transactions-ui/specs/ui/` hasta que
> `sdd-archive` lo promueva a la ubicación canónica
> `openspec/specs/ui/spec.md`. El canónico y el delta se mantienen
> en lockstep; el canónico es la fuente de verdad.
