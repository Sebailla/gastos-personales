...
- **Malas**: v5 todavía es beta; la superficie de la API puede cambiar entre betas. Pineamos la versión exacta en `package.json` y usamos `pnpm install --frozen-lockfile` en CI. El bug de resolución de módulos entre next-auth y Next.js (issue #18) se reportó contra `next@15.1.0` y `next-auth@5.0.0-beta.25` (ambos betas anteriores al import de `next/server` con extensión explícita y al campo `exports` de `next`). El workaround en las Slices A/B fue un bloque `test.exclude` en `vitest.config.ts` listando los 3 archivos de test que transitivamente importan `next-auth`. La Slice C-1 (commit `f055938`, PR #19) cerró el bug en la frontera upstream subiendo `next-auth` a `5.0.0-beta.31` y `next` a `16.2.9`, y removió las entradas de `test.exclude`. Verificado el 2026-06-21: `SKIP_TIMING=true pnpm test` pasa end-to-end (392/392) en un checkout limpio contra `next@16.2.9` + `next-auth@5.0.0-beta.31` sin errores de resolución de módulos de `next-auth` ni `next/server`. Nunca existió un stub de `resolve.alias` para `next-auth`; la redacción previa del ADR confundía el workaround de `test.exclude` con un parche de `resolve.alias`.
